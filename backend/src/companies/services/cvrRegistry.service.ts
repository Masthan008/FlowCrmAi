export interface CVRRecord {
  ...
  name: string;
  legalName: string;
  companyType: string; // e.g. A/S, ApS
  industry: string;
  ...
  foundedYear: number;
  vatNumber: string;
  status: string; // Active, Normal, Under Liquidation
}

const CVR_SIMULATOR_REGISTRY: CVRRecord[] = [
  {
    cvrNumber: '54643118',
...
export const cvrRegistryService = {
  searchCVR: async (query: string): Promise<CVRRecord[]> => {
    if (!query || query.trim().length === 0) {
      return CVR_SIMULATOR_REGISTRY;
    }

    const q = query.trim().toLowerCase();
    const results = CVR_SIMULATOR_REGISTRY.filter(item => 
      item.cvrNumber.toLowerCase().includes(q) ||
