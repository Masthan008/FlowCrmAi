...
export const companyKYCRepository = {
  findByCompanyId: async (companyId: string) => {
    let kyc = await prisma.companyKYC.findUnique({
      where: { companyId },
    });

    if (!kyc) {
      // Seed default KYC record if not present
      kyc = await prisma.companyKYC.create({
        data: {
          companyId,
          status: 'Pending',
          riskRating: 'Low',
  ...
  upsertKYC: async (companyId: string, data: any) => {
    const existing = await prisma.companyKYC.findUnique({ where: { companyId } });
