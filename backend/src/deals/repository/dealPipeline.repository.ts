...
const dealIncludeKanban = {
  ...
  dealActivities: { select: { id: true }, take: 1, orderBy: { createdAt: 'desc' as const } },
...
export class DealPipelineRepository {
  ...
  async duplicatePipeline(id: string, createdBy?: string) {
    ...
    return this.createPipeline({
      ...
      color: source.color,
      createdBy,
      stages: source.stages.map((s: any) => ({
        name: s.name,
        order: s.order,
  ...
  async getForecastData(pipelineId?: string) {
    ...
    for (let m = 0; m < 6; m++) {
      ...
      const monthDeals = openDeals.filter(d =>
        d.expectedCloseDate && new Date(d.expectedCloseDate) >= monthStart && new Date(d.expectedCloseDate) <= monthEnd
      );
      const monthWon = wonDeals.filter(d =>
        d.actualCloseDate && new Date(d.actualCloseDate) >= monthStart && new Date(d.actualCloseDate) <= monthEnd
      );
    ...
    for (let q = 0; q < 4; q++) {
      ...
      const qDeals = openDeals.filter(d =>
        d.expectedCloseDate && new Date(d.expectedCloseDate) >= qStart && new Date(d.expectedCloseDate) <= qEnd
      );
      const qWon = wonDeals.filter(d =>
        d.actualCloseDate && new Date(d.actualCloseDate) >= qStart && new Date(d.actualCloseDate) <= qEnd
      );
  ...
  async getAnalyticsData(pipelineId?: string) {
    ...
    // Owner Performance
    const ownerMap = new Map<string, { name: string; deals: number; value: number; won: number }>();
    deals.forEach(d => {
  ...
  async getPipelineHealth(pipelineId?: string) {
    ...
    let healthScore = 100;
    if (aging.stalled > 0) healthScore -= Math.min(25, aging.stalled * 5);
    if (aging.overdue > 0) healthScore -= Math.min(25, aging.overdue * 8);
    if (aging.atRisk > 0) healthScore -= Math.min(15, aging.atRisk * 3);
    if (kpis.winRate < 20) healthScore -= 15;
    if (kpis.avgSalesCycle > 90) healthScore -= 10;
    healthScore = Math.max(0, healthScore);

    let healthLabel = 'Excellent';
    if (healthScore < 30) healthLabel = 'Critical';
    else if (healthScore < 50) healthLabel = 'Poor';
    else if (healthScore < 70) healthLabel = 'Fair';
  ...
  async getQuotas(filters?: any) {
    ...
    return prisma.dealQuota.findMany({
      where,
      include: {
  ...
  async createQuota(data: any) {
    return prisma.dealQuota.create({ data });
  }

  async updateQuota(id: string, data: any) {
    return prisma.dealQuota.update({ where: { id }, data });
  }

  async deleteQuota(id: string) {
    return prisma.dealQuota.update({
      where: { id },
    ...
    deals.forEach(d => {
      ...
      if (d.status === 'Won') {
        ...
        if (d.actualCloseDate) {
          entry.totalCycleDays += Math.max(1, Math.floor(
            (new Date(d.actualCloseDate).getTime() - new Date(d.createdAt).getTime()) / 86400000
          ));
        }
      }
      if (d.status === 'Lost') entry.lostDeals++;
  ...
  async getSavedViews(employeeId?: string) {
    ...
    return prisma.dealPipelineView.findMany({
      where,
  ...
  async createView(data: any) {
    return prisma.dealPipelineView.create({ data });
  }

  async deleteView(id: string) {
    return prisma.dealPipelineView.update({
      where: { id },
      data: { deletedAt: new Date() },
    });
  }
...
export const dealPipelineRepository = new DealPipelineRepository();
export default dealPipelineRepository;
