...
async function runTests() {
  ...
  try {
    const adminSession = await authService.login(
      { email: 'admin@flowcrm.ai', password: 'Password@123' },
      { ipAddress: '127.0.0.1', userAgent: 'TestRunner Browser/Chrome' }
    );
  ...
  try {
    const loginResult = await authService.login(
      { email: testEmail, password: 'StrongPassword@12345' },
      { ipAddress: '192.168.1.100', userAgent: 'Mozilla/Safari/iPhone', browser: 'Safari', os: 'iOS', device: 'Mobile' }
    );
    ...
    const refreshResult = await authService.refresh(loginResult.refreshToken, { ipAddress: '192.168.1.100' });
    ...
    try {
      await authService.refresh(loginResult.refreshToken, { ipAddress: '192.168.1.100' });
  ...
  try {
    const resetToken = await authService.forgotPassword(testEmail);
    ...
    const newLogin = await authService.login(
      { email: testEmail, password: 'BrandNewSecurePassword@9876' },
      { ipAddress: '127.0.0.1' }
    );
