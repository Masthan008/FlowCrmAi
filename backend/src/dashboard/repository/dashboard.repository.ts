...
export class DashboardRepository {
   ...
   */
  getDateRange(timeframe: string, customStart?: string, customEnd?: string) {
    const now = new Date();
  ...
  async getRevenueAnalytics() {
    ...
    const [
      ...
      prisma.payment.aggregate({ _sum: { amount: true }, where: { status: 'success', createdAt: { gte: startOfWeek } } }),
      prisma.payment.aggregate({ _sum: { amount: true }, where: { status: 'success', createdAt: { gte: startOfMonth } } }),
      prisma.payment.aggregate({ _sum: { amount: true }, where: { status: 'success', createdAt: { gte: startOfQuarter } } }),
      prisma.payment.aggregate({ _sum: { amount: true }, where: { status: 'success', createdAt: { gte: startOfYear } } }),
      prisma.deal.aggregate({ _avg: { value: true }, where: { stage: { name: { in: ['Won', 'Closed Won'] } }, deletedAt: null } })
    ]);
  ...
  async getTeamLeaderboard() {
    ...
    return employees.map((emp) => {
      const wonDeals = emp.deals.filter((d: any) => d.stage?.name.includes('Won'));
  ...
  async getGoals() {
    ...
    const [closedDealsCount, monthlyRevenue] = await Promise.all([
      prisma.deal.count({ where: { stage: { name: { in: ['Won', 'Closed Won'] } }, updatedAt: { gte: startOfMonth }, deletedAt: null } }),
      prisma.payment.aggregate({ _sum: { amount: true }, where: { status: 'success', createdAt: { gte: startOfMonth } } })
    ]);
  ...
  async getKPIs() {
    ...
    const [
      ...
      prisma.deal.count({ where: { deletedAt: null, stage: { name: { notIn: ['Won', 'Lost', 'Closed Won', 'Closed Lost'] } } } }),
      prisma.deal.count({ where: { deletedAt: null, stage: { name: { in: ['Won', 'Closed Won'] } } } }),
      prisma.deal.count({ where: { deletedAt: null, stage: { name: { in: ['Lost', 'Closed Lost'] } } } }),
      prisma.payment.aggregate({ _sum: { amount: true }, where: { status: 'success', createdAt: { gte: startOfMonth } } }),
      prisma.task.count({ where: { deletedAt: null, status: { notIn: ['completed', 'done', 'Closed'] } } }),
    ]);
   ...
   * Chart trends helper
   */
  async getCharts(timeframe: string) {
    const now = new Date();
    let startDate = new Date();
    ...
    const [leads, deals, payments] = await Promise.all([
      prisma.lead.findMany({ where: { createdAt: { gte: startDate }, deletedAt: null }, select: { createdAt: true } }),
      prisma.deal.findMany({ where: { createdAt: { gte: startDate }, deletedAt: null }, select: { createdAt: true, value: true } }),
      prisma.payment.findMany({ where: { createdAt: { gte: startDate }, status: 'success' }, select: { createdAt: true, amount: true } }),
    ]);
