...
exports.leadAutomationService = void 0;
...
exports.leadAutomationService = {
    ...
    assignLead: async (params) => {
        ...
        await db_1.prisma.leadAssignment.create({
            data: {
                leadId,
                assignedToId,
                assignmentType: type,
        ...
        await db_1.prisma.leadNotification.create({
            data: {
                userId: assignedToId,
        ...
        await exports.leadAutomationService.executeWorkflows('Lead Assigned', updatedLead, userId);
        return updatedLead;
    },
    /**
     * Auto routing engines (Round Robin or Load-Based workload distribution)
     */
    autoAssignLead: async (leadId, ruleType, userId) => {
        const employees = await db_1.prisma.employee.findMany({ where: { deletedAt: null } });
        if (employees.length === 0)
            throw new Error('No active employees to assign to.');
        let targetEmployeeId = employees[0].id;
        if (ruleType === 'ROUND_ROBIN') {
            // Find the last assignment log to locate who was assigned last
            const lastAssignment = await db_1.prisma.leadAssignment.findFirst({
            ...
            const workloads = await db_1.prisma.lead.groupBy({
                by: ['assignedToId'],
                _count: { id: true },
        ...
        return exports.leadAutomationService.assignLead({
            leadId,
     ...
     */
    convertLead: async (leadId, config, userId) => {
    ...
    calculateLeadScore: async (leadId) => {
        ...
        const dbScore = await db_1.prisma.leadScore.upsert({
            where: { leadId },
     ...
     */
    evaluateLeadHealth: async (leadId) => {
        const lead = await db_1.prisma.lead.findUnique({
            where: { id: leadId },
            include: {
                score: true,
        ...
        const health = await db_1.prisma.leadHealth.upsert({
            where: { leadId },
            update: {
                leadScore,
                activityScore,
     ...
     */
    updateSlaStatus: async (leadId, responseMinutes) => {
        let status = 'Green';
        ...
        const sla = await db_1.prisma.leadSla.upsert({
            where: { leadId },
            update: {
     ...
     */
    executeWorkflows: async (trigger, lead, userId) => {
        const workflows = await db_1.prisma.leadWorkflow.findMany({
        ...
        for (const wf of workflows) {
            ...
            let isMatch = true;
            if (conditions.priority && lead.priority !== conditions.priority)
                isMatch = false;
            if (conditions.industry && lead.industry !== conditions.industry)
                isMatch = false;
            if (conditions.minValue && (lead.value || 0) < conditions.minValue)
                isMatch = false;
            if (isMatch) {
                const actions = wf.actions || [];
                for (const action of actions) {
                    if (action.type === 'Assign User') {
                        await exports.leadAutomationService.assignLead({
                            leadId: lead.id,
...
exports.default = exports.leadAutomationService;
