...
exports.branchService = {
    getBranches: async (companyId) => {
        return branch_repository_1.branchRepository.findByCompanyId(companyId);
    },
    getBranch: async (branchId) => {
        const branch = await branch_repository_1.branchRepository.findByIdWithCompany(branchId);
        if (!branch)
            throw Object.assign(new Error('Branch not found'), { statusCode: 404 });
        return branch;
    },
    createBranch: async (companyId, data, userId) => {
        const branch = await branch_repository_1.branchRepository.create({
            companyId,
            name: data.name,
    ...
    updateBranch: async (branchId, data, companyId, userId) => {
    ...
    deleteBranch: async (branchId, companyId, userId) => {
        const existing = await branch_repository_1.branchRepository.findById(branchId);
        if (!existing)
            throw Object.assign(new Error('Branch not found'), { statusCode: 404 });
        await branch_repository_1.branchRepository.softDelete(branchId, userId || null);
        await companyTimeline_service_1.companyTimelineService.logEvent({
            companyId,
            type: 'BRANCH_DELETED',
