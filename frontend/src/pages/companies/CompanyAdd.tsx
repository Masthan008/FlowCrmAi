...
const companyFormSchema = z.object({
  ...
  country: z.string().max(100).refine(val => !val || textOnlyRegex.test(val), 'Country must contain only letters').optional().or(z.literal('')),
  state: z.string().max(100).refine(val => !val || textOnlyRegex.test(val), 'State must contain only letters').optional().or(z.literal('')),
  city: z.string().max(100).refine(val => !val || textOnlyRegex.test(val), 'City must contain only letters').optional().or(z.literal('')),
  postalCode: z.string().max(20).refine(val => !val || /^[0-9]+$/.test(val), 'Postal Code must contain only numbers').optional().or(z.literal('')),
