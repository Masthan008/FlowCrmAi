...
import { BarChart3, DollarSign, Target, TrendingUp, Users, Loader2 } from 'lucide-react';
...
export const Reports: React.FC = () => {
  ...
  useEffect(() => {
    const fetchOverview = async () => {
      try {
        setLoading(true);
        const res = await api.get('/reports/sales-overview');
        if (res.data?.success) {
    ...
    <div className="space-y-6 text-slate-700 text-xs">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center min-h-[300px] gap-2">
          ...
          <p className="text-xs text-slate-400 font-semibold">Computing live sales reports...</p>
        </div>
      ) : (
        <>
          {/* Overview Cards Grid */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 select-none">
            <div className="glass-card p-5 space-y-2">
              ...
              <h2 className="text-2xl font-black text-slate-800 dark:text-slate-200">${data.totalRevenue.toLocaleString()}</h2>
              <span className="text-[10px] text-emerald-600 font-bold">Live database revenue total</span>
            </div>
            ...
            <div className="glass-card p-5 space-y-2">
              ...
              <h2 className="text-2xl font-black text-slate-800 dark:text-slate-200">{data.winRate}%</h2>
              <span className="text-[10px] text-slate-400 font-semibold">Won vs closed opportunities</span>
            </div>
            ...
            <div className="glass-card p-5 space-y-2">
              ...
              <h2 className="text-2xl font-black text-slate-800 dark:text-slate-200">${data.activePipeline.toLocaleString()}</h2>
              <span className="text-[10px] text-amber-600 font-bold">Unclosed pipeline value</span>
            </div>
            ...
            <div className="glass-card p-5 space-y-2">
              ...
              <h2 className="text-2xl font-black text-slate-800 dark:text-slate-200">{data.execCount} Members</h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="glass-card p-6 lg:col-span-2 space-y-4">
              <h3 className="font-bold text-slate-855 dark:text-slate-100 text-sm">Monthly Performance Leaderboard</h3>
            ...
            <div className="glass-card p-6 space-y-4">
              <h3 className="font-bold text-slate-855 dark:text-slate-100 text-sm">Lead Conversion Funnel</h3>
